    ...
    def on_forward(self, task: "tasks.ClassyTask") -> None:
        ...
        if isinstance(model_output, list):
            ...
            if isinstance(model_output, dict):
                projs = torch.cat(model_output['projs'][0])
                scores = torch.cat(model_output['scores'][0])
                proj_has_nan = not torch.isfinite(projs).all()
                scores_has_nan = not torch.isfinite(projs).all()
                if proj_has_nan or scores_has_nan:
                    has_nan = True
                else:
                    has_nan = False
            else:
