...
class SSLTensorboardHook(ClassyHook):
    ...
    def on_forward(self, task: "tasks.ClassyTask") -> None:
        ...
        if (self.log_params and self.log_params_every_n_iterations > 0
                and task.train
            ...
            if (task.loss.loss_config.name == 'dense_swav_loss'
                    and self.log_embs):
                # TODO: Parametrize in config
                tb_config = task.config.HOOKS.TENSORBOARD_SETUP
                queue_sampling_N = tb_config.QUEUE_SAMPLING_N
                emb_ratio = tb_config.PARAMS_PER_EMB_RATIO
                log_embs_every_n_iterations = emb_ratio * self.log_params_every_n_iterations
                if task.iteration % log_embs_every_n_iterations == 0:
                    # Visualize current prototype vector distribution
                    prototypes = task.base_model.heads[0].prototypes0.weight
                    self.tb_writer.add_embedding(prototypes,
                                                 global_step=task.iteration,
                                                 tag="prototypes")
                    # Visualize current projection vector distribution in queue
                    if task.loss.loss_config.queue.local_queue_length > 0:
                        queue = getattr(task.loss.dense_swav_criterion,
                                        "local_queue" + str(0))[0]
                        # Randomly sample from queue to reduce memory
                        queue_N = queue.shape[0]
                        uniform_prob = torch.ones(queue_N, dtype=torch.float)
                        idxs = torch.multinomial(uniform_prob,
                                                 queue_sampling_N)
                        queue = queue[idxs]
                        self.tb_writer.add_embedding(
                            queue, global_step=task.iteration, tag="queue")
    ...
    def on_update(self, task: "tasks.ClassyTask") -> None:
        ...
        if (self.log_params_every_n_iterations > 0
                and self.log_params_gradients and task.train
        ...
        if iteration % task.config["LOG_FREQUENCY"] == 0 or (
            ...
            pic_per_batch_per_gpu_per_sec = (int(pic_per_batch_per_gpu /
                                                 batch_time_avg_s)
                                             if batch_time_avg_s > 0 else 0.0)
            self.tb_writer.add_scalar(
