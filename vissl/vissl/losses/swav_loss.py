...
class SwAVLoss(ClassyLoss):
    ...
    def forward(self, output: torch.Tensor, target: torch.Tensor):
        """
        N = batch_size_per_replica (2) * views (8)

        Args;
            output (list): [0] Projections Z: torch.Tensor (N, D)
                           [1] Scores C^T Z: torch.Tensor (N, K)
            target (torch.Tensor): List of integers w. dim (N) <-- ignored
        """
...
class SwAVCriterion(nn.Module):
    ...
    def forward(self, scores: torch.Tensor, head_id: int):
        ...
        # Batch size == input images per replica
        bs = scores.shape[0] // self.num_crops  # <-- Floor division
