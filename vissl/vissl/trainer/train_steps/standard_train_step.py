...
def standard_train_step(task):
    ...
    # Sample (dict): Same view-wise ordering as 'dense_collator' output
    sample = construct_sample_for_model(sample, task)
    # Sample (dict): Renamed keys, top list removed, and moved to device memory
    #     'input' (list): List with #views of torch.Tensor views.
    #                     [ view_1 (img_N, 3, h, w), ... , view_M (img_N, 3, h, w) ]
    #     'target' (torch.Tensor): Common region coordinates (#views*#imgs, 2, 2)
    #                              tl, br = label[view_idx*img_N + img_idx]

    #debug_func.plot_sample_for_model(sample)

    # Only need gradients during training
    grad_context = torch.enable_grad() if task.train else torch.no_grad()
    ddp_context = (task.model.no_sync()
                   if task.enable_manual_gradient_reduction else
                   contextlib.suppress())
    torch_amp_context = (torch.cuda.amp.autocast() if task.amp_type
    ...
    with grad_context, ddp_context, torch_amp_context:
        ...
        with PerfTimer("forward", perf_stats), record_function("forward"):
            if task.enable_manual_gradient_reduction:
                ...
                # input:  tensor (N, view_M, 3, h, w)
                # target: tensor (N, view_M, 2, 2)
            # Dense SSL samples have coordinate labels
            if sample['target'].shape[-1] == 2:
                model_output = task.model(sample)
            # Ordinary SSL task
            else:
                model_output = task.model(sample["input"])

        ################################################################
        # SwAV model_output: [ [tensor (N, D), tensor (N, K) ] ]
        #                       Z (16, 128)    Score C^T Z (16, 3000)
        ################################################################
        ...
        label_superpixels = sample["superpixels"][0]  # List --> torch.Tensor

        # Run hooks on forward pass
        task.run_hooks(SSLClassyHookFunctions.on_forward.name)

        # Compute loss
        # TODO: Remove 'label_superpixels' as exist in 'model_output["region"]'
        with PerfTimer("loss_compute",
                       perf_stats), record_function("loss_compute"):
            local_loss = task.loss(model_output, label_superpixels)
