...
def infer_losses_config(cfg):
    ...
    # some inference for the Dense SwAV loss.
    if cfg.LOSS.name == "dense_swav_loss":
        assert len(cfg.MODEL.HEAD.PARAMS) == 1
        assert cfg.MODEL.HEAD.PARAMS[0][0] in {"dense_swav_head"}
        ...
        cfg.LOSS.dense_swav_loss.num_prototypes = cfg.MODEL.HEAD.PARAMS[0][1]["num_clusters"]
        cfg.LOSS.dense_swav_loss.embedding_dim = cfg.MODEL.HEAD.PARAMS[0][1]["dims"][-1]
        cfg.LOSS.dense_swav_loss.num_crops = total_num_crops or cfg.LOSS.dense_swav_loss.num_crops
        from vissl.utils.checkpoint import get_checkpoint_folder

        cfg.LOSS.dense_swav_loss.output_dir = get_checkpoint_folder(cfg)
        world_size = cfg.DISTRIBUTED.NUM_NODES * cfg.DISTRIBUTED.NUM_PROC_PER_NODE
        batch_size = cfg.DATA.TRAIN.BATCHSIZE_PER_REPLICA
        batch_size *= world_size
        queue_length = cfg.LOSS.dense_swav_loss.queue.queue_length
        queue_length -= queue_length % batch_size
        cfg.LOSS.dense_swav_loss.queue.queue_length = queue_length
        cfg.LOSS.dense_swav_loss.queue.local_queue_length = queue_length // world_size
